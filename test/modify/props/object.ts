...
await test('modify object', async (t) => {
  ...
  deepEqual((await db.query('thing', id1).get()).info, {
    count: 20,
    title: '',
  })
...
await test('modify object on edge', async (t) => {
  ...
  const res1 = await db.query('holder', id1).include('toThing.$edgeInfo').get()
  ...
  const res2 = await db.query('holder', id1).include('toThing.$edgeInfo').get()
