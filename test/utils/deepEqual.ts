...
await test('deepEqual 2', async (t) => {
  const bla = {
    id: 213891765,
...
await test('deepEqual 3', async (t) => {
  const bla = {
    id: 213906207,
  ...
  assert(notEqual(bla, blarf))
})

await test('deepEqual 4', async (t) => {
  const bla = {
    id: 213913182,
...
await test('deepEqual _keys', async (t) => {
  const bla = {
    hello: 'super cool',
    what: {
      nested: 'yes',
  ...
  assert(deepEqual(bla, blarf), 'same object with _key')
  blarf._niceKey = 2
  assert(notEqual(bla, blarf), 'change _key value')
})
