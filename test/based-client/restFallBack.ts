...
import { encodeAuthState } from '../../src/client/index.js'
import { concatUint8Arr, readUint32 } from '../../src/utils/index.js'
import {
  encodeFunctionMessage,
  encodeObserveMessage,
} from '../../src/client/outgoing/protocol.js'
import { decodeHeader } from '../../src/server/protocol.js'
import { FunctionServerType, genObserveId } from '../../src/protocol/index.js'
import { parseIncomingData } from '../../src/client/incoming/parseIncomingData.js'
...
test('rest fallback', async (t: T) => {
  ...
  if (type === FunctionServerType.subscribe) {
    const start = 20
    const end = len + 4
    if (len !== 16) {
