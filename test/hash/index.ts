...
import { equal } from '../shared/index.js'

await test('hash', async (t) => {
  const a = { x: true }
  assert(hash(a) > 0)
  const bla = {
    x: {
...
await test('hash array', async (t) => {
  ...
  while (cnt < 1e6) {
    ...
    if (set[x] && set[x] !== str) {
      throw new Error(`'Colision ${str} '-->', ${set[x]} hash: ${x}`)
    }
    set[x] = str
    cnt++
  }
...
await test('hash stress many arrays', async (t) => {
  const a: number[] = []
  for (let i = 0; i < 6; i++) {
...
await test('hash stress object with many keys', async (t) => {
  const a: any = {}
  for (let i = 0; i < 1e6; i++) {
...
await test('hash colish', async (t) => {
  const prevs: any[] = []
  ...
  for (let i = 0; i < 2; i++) {
    ...
    while (cnt < 1e6) {
      ...
      if (prev) {
        if (prev !== ip) {
          throw new Error(`Colish ${ip} ${prev} hash ${x} #${ipCnt}`)
        }
      }
      cnt++
...
await test('hash  hashObjectIgnoreKeyOrder', async (t) => {
  const a = {
    a: true,
    b: true,
    c: {
  ...
  equal(hashObjectIgnoreKeyOrder(a), hashObjectIgnoreKeyOrder(b))
})

await test('hash  hashObjectIgnoreKeyOrder large', async (t) => {
  const a = {
    children: [
      {
        type: 'waitingScreen',
...
await test('hash  hashObjectIgnoreKeyOrder large 2', async (t) => {
  const a = {
    children: [
      {
        type: 'waitingScreen',
...
await test('hash  hashObjectIgnoreKeyOrder large 3.1', async (t) => {
...
await test('hash  hashObjectIgnoreKeyOrder large 3', async (t) => {
  const a = {
    type: 'multipleChoice',
    disabled: false,
    children: [
...
await test('hash  hashObjectIgnoreKeyOrder large 4', async (t) => {
  const a = {
    children: [
      {
        type: 'waitingScreen',
...
await test('hash stress hashObjectIgnoreKeyOrder', async (t) => {
  const a: any = {}
...
await test('hash test equality 1', async (t) => {
  const a = {
    type: 'folder',
    title: '',
    id: 'fo1',
  ...
  assert(hashA1 !== hashB1)
  assert(hashA !== hashB)
})

await test('hash test equality 2', async (t) => {
  const a = {
    type: 'folder',
    title: '',
    id: 'fo1',
...
await test('hash test equality 3', async (t) => {
  const a = {
    type: 'videoScreen',
    index: 0,
    id: '309aa290aa',
...
await test('hash test equality 4', async (t) => {
  const a = {
    type: 'videoScreen',
    index: 0,
    id: '309aa290aa',
...
await test('hash test equality 5', async (t) => {
  const a = {
    type: 'videoScreen',
    index: 0,
    id: '309aa290aa',
...
await test('hash test equality 6', async (t) => {
  const a = {
    buttonText: 'aax',
  }
  const b = {
...
await test('hash test equality 7', async (t) => {
  const a = {
    type: 'videoScreen',
    index: 0,
    id: '309aa290aa',
...
await test('hash fixed length', async (t) => {
  const texts: any[] = []
  for (let i = 0; i < 10000; i++) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    equal(x.toString().length, 15)
    equal(y.length, 9)
    equal(a.length, 10)
    equal(z.length, 6)
    equal(blap.length, 20)
    equal(blurp.length, 10)
  }
})

await test('hash fixed length ignore key order', async (t) => {
  const a = hashCompact(
    {
      x: 1,
      y: 2,
...
await test('hash ingore key order test collision', async (t) => {
  ...
  for (const o of arr) {
    ...
    if (unique.has(x)) {
      throw new Error(`Hash collision! ${x} @ number ${i}`)
    }
    if (x < 0) {
      negative.add(x)
    }
