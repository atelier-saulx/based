...
import type { BasedClient, ClientQueryMap } from '../client/index.js'
...
/**
  ...
  ```
*/
export const useQuery = <N extends keyof ClientQueryMap>(
  name: N,
  payload?: ClientQueryMap[N]['payload'],
  opts?: {
    persistent: boolean
  },
): {
  loading: boolean
  data?: ClientQueryMap[N]['result']
  error?: BasedError
  checksum?: number
} => {
  const client: BasedClient = useContext(Ctx)
