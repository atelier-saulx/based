...
pub fn start(env: c.napi_env, info: c.napi_callback_info) callconv(.C) c.napi_value {
...
pub fn stop(env: c.napi_env, info: c.napi_callback_info) callconv(.C) c.napi_value {
    return stopInternal(env, info) catch return null;
}
...
fn startInternal(env: c.napi_env, info: c.napi_callback_info) !c.napi_value {

    // does this make double things with valgrind? Ask marco
    dbCtx.init();
    const ctx = try dbCtx.createDbCtx(env, info);
    ctx.selva = selva.selva_db_create();
    var externalNapi: c.napi_value = undefined;
    ctx.initialized = true;
    _ = c.napi_create_external(env, ctx, null, null, &externalNapi);
    return externalNapi;
}

fn stopInternal(env: c.napi_env, info: c.napi_callback_info) !c.napi_value {
