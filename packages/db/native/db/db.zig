...
pub fn initReadTxn() !*c.MDB_txn {
    if (dbCtx.ctx.readTxnCreated) {
        return dbCtx.ctx.readTxn;
    }
    dbCtx.ctx.readTxnCreated = true;
...
pub fn createTransaction(comptime readOnly: bool) !?*c.MDB_txn {
    ...
    if (readOnly == true) {
        try errors.mdb(c.mdb_txn_begin(dbCtx.ctx.env, null, c.MDB_RDONLY, &txn));
    } else {
        try errors.mdb(c.mdb_txn_begin(dbCtx.ctx.env, null, 0, &txn));
    }
    return txn.?;
}

pub inline fn getName(typeId: dbCtx.TypeId, field: u8, shard: u16) dbCtx.DbName {
    const s: dbCtx.TypeId = @bitCast(shard);
    if (s[0] == 0 and s[1] != 0) {
...
pub fn getReadShard(dbiName: dbCtx.DbName, queryId: u32) !dbCtx.Shard {
    var s = dbCtx.ctx.readShards.get(dbiName);
    if (s == null) {
        var cursor: ?*c.MDB_cursor = null;
        const dbi = try openDbi(false, dbiName, dbCtx.ctx.readTxn);
        errors.mdb(c.mdb_cursor_open(dbCtx.ctx.readTxn, dbi, &cursor)) catch |err| {
            std.log.err("Cannot open cursor {any}\n", .{err});
            return err;
        };
        s = .{ .dbi = dbi, .key = dbiName, .cursor = cursor, .queryId = queryId };
        dbCtx.ctx.readShards.put(dbiName, s.?) catch |err| {
...
pub fn openShard(comptime create: bool, dbiName: dbCtx.DbName, txn: ?*c.MDB_txn) !dbCtx.Shard {
    ...
    const s: dbCtx.Shard = .{ .dbi = dbi, .key = dbiName, .cursor = cursor, .queryId = null };
    return s;
}

pub inline fn closeShard(shard: dbCtx.Shard) void {
    c.mdb_cursor_close(shard.cursor);
}

pub inline fn closeCursor(shard: dbCtx.Shard) void {
    c.mdb_cursor_close(shard.cursor);
}
...
pub inline fn readField(id: u32, shard: dbCtx.Shard) []u8 {
...
pub fn getField(id: u32, field: u8, typeId: dbCtx.TypeId, currentShard: u16, queryId: u32) []u8 {
...
pub fn writeField(id: u32, buf: []u8, shard: dbCtx.Shard) !void {
    var k: c.MDB_val = .{ .mv_size = 4, .mv_data = @constCast(&id) };
    var v: c.MDB_val = .{ .mv_size = buf.len, .mv_data = buf.ptr };
    try errors.mdb(c.mdb_cursor_put(shard.cursor, &k, &v, 0));
}

pub fn deleteField(id: u32, shard: dbCtx.Shard) ![]u8 {
    var k: c.MDB_val = .{ .mv_size = 4, .mv_data = @constCast(&id) };
    var v: c.MDB_val = .{ .mv_size = 0, .mv_data = null };
