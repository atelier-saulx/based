...
static struct SelvaNode *del_single_ref(struct SelvaDb *db, struct SelvaNode *src_node, const struct EdgeFieldConstraint *efc, struct SelvaFields *fields, struct SelvaFieldInfo *nfo, bool ignore_dependent)
{
    ...
    memset(vp, 0, sizeof(struct SelvaNode *)); /* This is fine here because we have a copy of the original struct. */
...
int fields_set2(struct SelvaDb *db, struct SelvaNode *node, const struct SelvaFieldSchema *fs, struct SelvaFields *fields, const void *value, size_t len)
{
    return fields_set(db, node, fs, fields, value, len);
}
