...
int selva_schema_set_type(struct SelvaDb *db, const void *buf, size_t len)
{
    struct SelvaNodeSchema *ns;
    int err;

    err = schemabuf_count_fields(&nr_fields, buf, len);
    if (err) {
        return err;
    }
    ns = selva_malloc(nr_fields * sizeof(SelvaNodeSchema));
