...
struct memmem_ctx {
    ...
    size_t n; /*!< Current byte index in the stream. */
    size_t match_begin;
    size_t match_len;
};

static int cb_memmem(void * restrict ctx, uint8_t * restrict buf, size_t len)
{
    struct memmem_ctx *c = (struct memmem_ctx *)ctx;
    size_t n = c->n;
...
libdeflate_memmem(struct libdeflate_decompressor *decompressor, struct libdeflate_block_state *state, const char *in_buf, size_t in_len, const void *needle_buf, size_t needle_len)
{
    ...
    do {
        ...
        result = 0;
        res = libdeflate_decompress_stream(decompressor, state, in_buf, in_len, cb_memmem, &ctx, &result);
    } while (res == LIBDEFLATE_INSUFFICIENT_SPACE && libdeflate_block_state_growbuf(state));

    if (res != LIBDEFLATE_SUCCESS || !result) {
        return -1;
    }

    return ctx.match_begin;
}
