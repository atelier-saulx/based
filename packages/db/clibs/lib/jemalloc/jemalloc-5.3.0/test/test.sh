...
for t in $@; do
  ...
  if [ -e "${t}.sh" ] ; then
    ...
    # is then used to set SELVA_MALLOC_CONF (thus allowing the
    # per test shell script to ignore the SELVA_ detail).
    enable_fill=1 \
    enable_prof=1 \
    . ${t}.sh && \
    export_malloc_conf && \
    $JEMALLOC_TEST_PREFIX ${t} /Users/youzi/dev/based/packages/db/clibs/lib/jemalloc/jemalloc-5.3.0/ /Users/youzi/dev/based/packages/db/clibs/lib/jemalloc/jemalloc-5.3.0/
  else
    export MALLOC_CONF= && \
    export_malloc_conf && \
    $JEMALLOC_TEST_PREFIX ${t} /Users/youzi/dev/based/packages/db/clibs/lib/jemalloc/jemalloc-5.3.0/ /Users/youzi/dev/based/packages/db/clibs/lib/jemalloc/jemalloc-5.3.0/
