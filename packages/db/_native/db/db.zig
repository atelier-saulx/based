...
// READ SHARDS (TODO: GLOBAL MAKE IN A TOP LEVEL STRUCTURE)
...
pub fn openDbi(comptime create: bool, name: [6]u8, txn: ?*c.MDB_txn) c.MDB_dbi {
    ...
    _ = c.mdb_dbi_open(txn, &name, flags, &dbi);
...
pub fn getReadShard(dbiName: DbName, queryId: u32) Shard {
    ...
    if (s == null) {
        ...
        const dbi = openDbi(false, dbiName, readTxn);
        _ = c.mdb_cursor_open(readTxn, dbi, &cursor);
...
pub fn openShard(comptime create: bool, dbiName: DbName, txn: ?*c.MDB_txn) !Shard {
    const dbi = openDbi(create, dbiName, txn);
    var cursor: ?*c.MDB_cursor = null;
