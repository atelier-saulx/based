...
await test('noLoadDumps', async (t) => {
  ...
  deepEqual(await db2.query('employee', 2).include((s) => s('subordinates').count()).get().inspect(), 750)

  await db2.query('employee', 2).include((s) => s('subordinates').count()).get().inspect()
