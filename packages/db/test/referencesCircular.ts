...
await test('e-com', async (t) => {
    ...
    // WARNING!: Running this (forcing the duplication of the already stabilished link between xiaomi and mirror)
    //  leads to a disastrous thread killing (SIGABRT)
    ...
    // console.log(
    //   '===>',
    //   (await db.query('product').include('*', 'buyers', 'sellers').get()).toObject(),
    // )
    ...
    console.log('===> 2')
    deepEqual(
      (await db.query('user').include('bought.name').get()).toObject(),
      [
        {
          id: 1,
          bought: [
