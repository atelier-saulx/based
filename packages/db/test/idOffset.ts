...
await test.skip('E-commerce Simulation', async (t) => {
  ...
  await db.drain()
  console.log((await db.query('product').range(0, 10_000_000).get()).length)
  console.log(
  ...
  await Promise.all(allQ)
  const allProducts = await db.query('product').range(0, 10_000_000).get()
  const finalProductCount = allProducts.length

  console.log(productIdsArr.length, finalProductCount, productIds)

  equal(finalProductCount, productIds)
  let id = 0
  for (const product of allProducts) {
    id++
    if (product.id !== id) {
      console.log('incorrect', product, 'expected', id)
