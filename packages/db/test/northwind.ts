...
await test('northwind', async (t) => {
  ...
  await Promise.all([
    ...
    orderDate: new Date(row[3]),
    requiredDate: new Date(row[4]),
    shippedDate: new Date(row[5]),
    shipVia: row[6],
    freight: row[7],
    shipName: row[8],
    shipAddress: row[9],
  ...
  //console.dir(await db.query('employees').include('firstName', 'territories').get().toObject(), { depth: 100 })
  //console.dir(await db.query('territories').include('territoryId', 'region', 'employees.id').get().toObject(), { depth: 100 })
  //console.dir(await db.query('employees')
  //      .include('firstName', (q) =>
  //        // TODO can we filter by region
  //        q('territories').include('territoryId').filter('id', '!=', 0),
  //      )
  //    .get().toObject(), { depth: 100 })
  //console.dir(await db.query('employees').include('firstName').filter('territories.region.regionDescription', '=', 'Eastern').get().toObject(), { depth: 100 })
})
