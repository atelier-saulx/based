...
await test.skip('northwind', async (t) => {
  ...
  ].forEach((row: any[]) => db.create('customers', {
    customerId: row[0],
    companyName: row[1],
    contactName: row[2],
    contactTitle: row[3],
  ...
  ;[
    ...
    [9, 'Dodsworth', 'Anne', 'Sales Representative', 'Ms.', '1966-01-27', '1994-11-15', '7 Houndstooth Rd.', 'London', '', 'WG2 7LT', 'UK', '(71) 555-4444', '452', '', 'Anne has a BA degree in English from St. Lawrence College.  She is fluent in French and German.', 5, 'http://accweb/emmployees/davolio.bmp'],
  ...
  ].forEach((row) => db.create('employees', {
    id: row[0],
    lastName: row[1],
    firstName: row[2],
    title: row[3],
    titleOfCourtesy: row[4],
    birthDate: new Date(row[5]),
    hireDate: new Date(row[6]),
    address: row[7],
    city: row[8],
    region: row[9],
    postalCode: row[10],
  ...
  ;[
    ...
    ['98052', 'Redmond', 2],
    ['98104', 'Seattle', 2],
  ].forEach((row) => db.create('territories', {
    territoryId: row[0],
    territoryDescription: row[1],
    region: row[2],
  }))
  ...
  console.dir(await db.query('employees').include('*', '**').get().toObject(), { depth: 100 })
  console.dir(await db.query('territories').include('**').get().toObject(), { depth: 100 })
})
