...
export function getWriteFuncs(buf: Buffer): {
	...
	return {
		...
		f: (v: bigint, offset: number): number =>
			buf.writeBigInt64BE(v, offset),
		pf: (
		...
		g: (v: bigint, offset: number): number =>
			buf.writeBigInt64LE(v, offset),
		pg: (
			v: bigint[] | null | undefined,
			offset: number,
			_typeSize: number,
		...
		m: (v: bigint, offset: number): number =>
		...
		n: (v: bigint, offset: number): number =>
