...
extern inline enum SelvaTypeBlockStatus selva_block_status_set(const struct SelvaTypeEntry *te, block_id_t block_i, enum SelvaTypeBlockStatus mask);
...
struct SelvaNodeRes selva_upsert_node2(struct SelvaTypeEntry *type, node_id_t node_id, struct SelvaNode *node)
{
    if (unlikely(node_id == 0)) {
        return (struct SelvaNodeRes){};
    }
