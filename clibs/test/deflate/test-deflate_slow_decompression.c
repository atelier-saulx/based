 ...
 */
static void
generate_empty_static_huffman_blocks(uint8_t *p, size_t len)
{
 ...
 */
static void
generate_empty_dynamic_huffman_blocks(uint8_t *p, size_t len)
{
	struct output_bitstream os = { .next = p, .end = p + len };
		   ...
		   uint8_t *out, size_t out_nbytes_avail)
{
	...
	printf("[%s, libdeflate]: %"PRIu64" KB/s\n", input_type,
	       timer_KB_per_s((uint64_t)in_nbytes * NUM_ITERATIONS, t));
...
static uint64_t
do_test_zlib(const char *input_type, const uint8_t *in, size_t in_nbytes,
	     uint8_t *out, size_t out_nbytes_avail)
{
	z_stream z;
	int res;
 ...
 */
static const uint8_t orig_repro[3962] =
