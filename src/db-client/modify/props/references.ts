...
export const writeReferences = (ctx: Ctx, def: PropDef, val: any) => {
  ...
  if (Array.isArray(val)) {
    ...
    /*
      ...
      createNode, 1, createProp, 'name', 'youzi', 

    */

    // overwrite
    // set: [RefOp.overwrite, idsLength, ...ids]
    // setWithIndex: [RefOp.update, idsLength, id, index, id, index, ...]

    // better:
    // set: [RefOp.set, idsLength, ...ids, RefOp.end]
    // set: [RefOp.set, idsLength, ...ids, RefOp.setWithIndex, id, index, id, index, ..., RefOp.end]
    // clear and set: [RefOp.clear, RefOp.set, idsLength, ...ids, RefOp.setWithIndex, id, index, id, index, ..., RefOp.end]
    // clear: [RefOp.clear, RefOp.end]
    ...
    /*
      ...
      set: [RefOp.set, 33, 1, RefOp.edge, id, ...update payload, RefOp.end]
    ...
    */
    ...
    // for (const item of val) {
    //   if (typeof item === 'number') {
    //     if (op !== RefOp.overwrite) {
    //       if (op) {
    //         ctx.buf[start] = op
    //         // writeUint32(ctx.buf, len, )
    //       }
    //       op = RefOp.overwrite
    //     }
    //     // ctx.buf[opIndex] = RefOp.overwrite
    //     writeU32(ctx, item)
    //   }
