...
const sendCacheSwapEncoding = async (
  ...
  try {
    ...
    session.res.cork(() => {
      ...
      session.res.writeStatus(status)
      if (encoding) {
        session.res.writeHeader('Content-Encoding', encoding)
      }
      session.res.writeHeader('ETag', String(checksum))
      end(ctx, payload)
    })
  } catch (err) {
...
const sendCache = (
  ...
  session.res.cork(() => {
    ...
    session.res.writeHeader('ETag', String(checksum))
    if (isDeflate) {
      session.res.writeHeader('Content-Encoding', 'deflate')
    }
