...
import type { BasedRoute, WebSocketSession } from '../../../functions/index.js'
...
> = (props) => {
  const id = props.id!
  if (hasChannel(props.server, id)) {
    subscribeChannel(props.server, id, props.ctx)
    return
  }
  ...
  if (!hasChannel(props.server, id)) {
    createChannel(props.server, props.route.name, id, props.payload, true)
  }
  subscribeChannel(props.server, id, props.ctx)
}
...
export const channelSubscribeMessage: BinaryMessageHandler = (
  ...
  if (
    rateLimitRequest(
      ...
      isChannelIdRequester
        ? (tmpRoute.rateLimitTokens ?? 0) * 5
        : (tmpRoute.rateLimitTokens ?? 0),
      server.rateLimit.ws,
    )
  ) {
    return false
