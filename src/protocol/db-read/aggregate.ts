...
export const readAggregate = (
  ...
  if (groupBy && groupBy.length > 0) {
    let cursor = offset
    while (cursor < len) {
      if (result[cursor] === 0 && result[cursor + 1] === 0 && !result[cursor + 2]) {
        // handle empty key edgecase if length 0 is ever written as 00
      }
      ...
      for (let i = 0; i < groupBy.length; i++) {
        ...
        if (i < groupBy.length - 1) {
          ...
          currentResultObj[key] = currentResultObj[key] || {}
          readAggValues(result, cursor + 2 + totalKeyLen, aggregates, currentResultObj[key])
        }
      }
