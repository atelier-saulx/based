...
pub inline fn iteratorNode(
    ...
    try include.include(node, ctx, includeQuery, typeEntry);
    // nodeCnt += 1;
    return true;
...
//         var it = try References.iterator(false, true, ctx.db, fromNode, header.prop, orginalTypeEntry);
...
//         }
//     } else {
//         var it = try References.iterator(false, false, ctx.db, fromNode, header.prop, orginalTypeEntry);
//         while (it.next()) |node| {
//             if (correctedForOffset != 0) {
//                 correctedForOffset -= 1;
//                 continue;
...
pub fn references(
    ...
    if (header.iteratorType == .edgeInclude) {
        var it = try References.iterator(false, true, ctx.db, fromNode, header.prop, orginalTypeEntry);
        const s = index.* + header.size - utils.sizeOf(t.QueryHeader);
        const edgeQuery = q[s .. s + header.edgeSize];
        const edgeTypeEntry = try Node.getType(ctx.db, header.edgeTypeId);
    ...
    } else if (header.iteratorType == .edge) {
        var it = try References.iterator(false, true, ctx.db, fromNode, header.prop, orginalTypeEntry);
        while (it.next()) |ref| {
            if (correctedForOffset != 0) {
                correctedForOffset -= 1;
