...
function find_first_frame(buf) {
  ...
  return [
    frame_hdr,
    frame,
    frame_hdr.frame_bsize < buf.length
      ? buf.slice(frame_hdr.frame_bsize)
      : null,
  ]
}
...
export async function connect(port, host) {
  return new Promise((resolveClient, rejectClient) => {
    ...
    socket.connect(port, host, () => {
      ...
      const reqWrap = (cmdId, cmdFn, ...args) =>
      ...
      reqWrap(CMD_ID_LSCMD, lscmd)
        .then((res) => {
          for (const [cmdId, cmdName] of res) {
            const prop = cmdName2prop(cmdName)
            commands[prop] = (buf) =>
              reqWrap(cmdId, sendMsg, Number(cmdId), buf)
      ...
      do {
        ...
        if (frame_hdr.flags & SELVA_PROTO_HDR_FREQ_RES) {
          if (frame_hdr.flags & SELVA_PROTO_HDR_FFIRST) {
            ...
            msgBuffers[frame_hdr.seqno] = Buffer.concat([
              msgBuffers[frame_hdr.seqno],
              frame.slice(2 * 8),
            ])
          }
          ...
          if (frame_hdr.flags & SELVA_PROTO_HDR_STREAM) {
            ...
            if (sub) {
              ...
              console.error(
                `Stream listener not found. seqno: ${frame_hdr.seqno}`
              )
            }
            ...
            if (req && req.cmdId === frame_hdr.cmd) {
              ...
              console.error(
                `Unexpected response! seqno: ${frame_hdr.seqno} cmd_id: ${frame_hdr.cmd}`
              )
